...
class PaymentGateway {
    constructor() {
        this.apiKey = 'YOUR_STRIPE_API_KEY'; // Replace with your actual API key
    ...
    calculateAmount() {
        // Get amount from the booking details
        const amountElement = document.getElementById('booking-amount');
        return amountElement ? parseInt(amountElement.value) * 100 : 0; // Convert to paise
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    paymentForm.addEventListener('submit', (e) => paymentGateway.handlePayment(e));
});
...
function handleUPIPayment() {
    ...
    const upiUrl = `upi://pay?pa=${upiId}&pn=BookPandit&am=${amount}&cu=INR`;
...
function handleNetBanking() {
    ...
    const amount = document.getElementById('booking-amount').value;
    const bankUrl = `/api/net-banking/${bankSelect.value}?amount=${amount}`;
...
function handleWalletPayment() {
    ...
    const amount = document.getElementById('booking-amount').value;
    const walletUrl = `/api/wallet/${walletSelect.value}?amount=${amount}`;
